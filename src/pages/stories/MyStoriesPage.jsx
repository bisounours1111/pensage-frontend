  ...
  useEffect(() => {
    const loadStories = async () => {
      try {
        ...
        // Stats likes/vues/commentaires en parallèle pour chaque histoire
        const withStats = await Promise.all(
          baseStories.map(async (s) => {
            const [likes, views, comments] = await Promise.all([
              likesApi.countLikes(s.id),
              historyApi.countViews(s.id),
              commentsApi.countByWebnovel(s.id, { rootOnly: true }),
            ]);
            return {
              ...s,
              stats: {
      ...
      <header className="p-6 md:p-12">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            style={{
              backgroundColor:
                filter === "all" ? colors.primary : colors.whiteTransparent,
          ...
          <button
            ...
            style={{
              backgroundColor:
                filter === "draft" ? colors.primary : colors.whiteTransparent,
      ...
      {!loading && !error && (
        <div className="space-y-8 px-6 md:px-12 pb-12">
          ...
          {draftStories.length > 0 && (
            <StoryRow title="Terminées" stories={draftStories} mode="manage" />
          )}
