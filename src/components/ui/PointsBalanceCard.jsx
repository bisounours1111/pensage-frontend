...
export default function PointsBalanceCard({ points, onWeekly, onAd, isPremium = false, canClaimWeekly = false }) {
  ...
  return (
    <div className="w-full rounded-2xl p-6 bg-white/30 backdrop-blur shadow-lg border border-white/20">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        <div>
          <p className="text-xl md:text-2xl font-bold" style={{ color: colors.text }}>Votre solde</p>

          <p className="text-2xl md:text-3xl font-semibold" style={{ color: colors.text }}>
          ...
          {!canClaimWeekly && (
            <p className="mt-1 text-sm opacity-75" style={{ color: colors.text }}>
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
          <button
            className={`px-4 py-2 rounded-lg text-white font-semibold shadow-md transition hover:scale-105 ${!canClaimWeekly ? 'opacity-50 cursor-not-allowed' : ''
              }`}
          ...
          >
            {canClaimWeekly ? (
              <>🎁 +{weeklyRewardAmount} pts hebdo</>
            ) : (
              <>⏰ +{weeklyRewardAmount} pts (déjà utilisé)</>
            )}
          </button>
          <button
